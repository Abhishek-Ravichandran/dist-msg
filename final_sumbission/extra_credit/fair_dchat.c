...
pthread_mutex_t fair_mutex_lock;
...
char* add_user_string_to_list(char* user_string) {
    ...
    newEntry->userObj->userQueueObj = (struct queueObj*)malloc(sizeof(struct queueObj));
    newEntry->userObj->userQueueObj->headMsg = NULL;
    sem_init(&(newEntry->userObj->userQueueObj->queue_sem), SHARED, 0);
...
void update_user_list(char* user_list_string) {
    ...
    pthread_mutex_lock(&(fair_mutex_lock));
    while(curr != NULL) {
        struct userListObj* temp = curr;
        curr = curr->next;
        free(temp->userObj->userQueueObj);
        free(temp->userObj);
        free(temp);
    }
    pthread_mutex_unlock(&(fair_mutex_lock));
...
void printFairQueues() {
    struct userListObj* curr = head;
    
    while (curr != NULL) {
        printf("PRINTING QUEUE FOR %s\n",curr->userObj->user_name);
        struct messageListObj *msgHead = curr->userObj->userQueueObj->headMsg;
        while (msgHead != NULL) {
            printf("message: %s\n",msgHead->msg);
...
void handle_fair_msg_request(char *str) {
    char* dup = strdup(str);
    char* msg = strtok(dup, "\n");
    int id_of_sender = atoi(strtok(NULL, "\n"));
    int sender_seq_no = atoi(strtok(NULL, "\n"));
    ...
    while(curr != NULL) {
        if(curr->userObj->user_id == id_of_sender) {
            if(curr->userObj->last_local_seq_no_rcvd + 1 == sender_seq_no) {
                curr->userObj->last_local_seq_no_rcvd = sender_seq_no;
                payload = getPayload("message\n", str);
                add_msg_to_queue(payload, curr->userObj->userQueueObj,0);
            }
...
void* checkFairQueues(void* args) {
    //printf("CHECKIGN FAIR QUEUE\n");
    struct queueObj* queue = args;
    ...
    while (1) {
        ...
        while (curr != NULL) {
            struct messageListObj *msgHead = curr->userObj->userQueueObj->headMsg;
            if (msgHead != NULL) {
                add_msg_to_queue(msgHead->msg,queue,0);
                pthread_mutex_lock(&(curr->userObj->userQueueObj->queue_mutex));
                curr->userObj->userQueueObj->headMsg = curr->userObj->userQueueObj->headMsg->next;
                pthread_mutex_unlock(&(curr->userObj->userQueueObj->queue_mutex));
            }
            curr = curr->next;
        } 
...
//  }
    ...
    local_user->userQueueObj = (struct queueObj*)malloc(sizeof(struct queueObj));
    local_user->userQueueObj->headMsg = NULL;
    sem_init(&(local_user->userQueueObj->queue_sem), SHARED, 0);
    ...
    if (pthread_create(&fairQueueThread, NULL, &checkFairQueues, (void *)broadcastQueue) != 0) {
        perror("Pthread Create");
        exit(1);
    }
        ...
        if (select_retval > 0) {
            ...
            if (FD_ISSET(local_socket_fd, &readset)) {
                ...
                if(strcmp(msg_type, "message-request") == 0) {
                    ...
                    //printf("I GOT MSG REQ\n");
                    handle_fair_msg_request(msg);
                }
