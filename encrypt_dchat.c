...
char* add_user_string_to_list(char* user_string) {
  ...
  newEntry->userObj->is_leader = atoi(strtok(NULL, "\n"));
    ...
    if(local_user->is_leader == 1) {
        char *enc_msg;
        asprintf(&enc_msg, "NOTICE %s joined on %s:%d", newEntry->userObj->user_name,inet_ntoa(newEntry->userObj->user_socket.sin_addr), ntohs(newEntry->userObj->user_socket.sin_port));
        encrypt_msg(enc_msg);
        asprintf(&payload, "message-request\n%s\n", enc_msg);
        //printf("PREPARED USER NOT: %s\n",payload);
    }
...
void handle_user_left(struct userListObj *curr, struct queueObj* queue) {
  ...
  char *enc_msg;
    asprintf(&enc_msg, "NOTICE %s left the chat or crashed", leftName);
    encrypt_msg(enc_msg);
    asprintf(&payload, "message-request\n%s\n", enc_msg);  
...
void* hold_election(void* args) {
    ...
    char *enc_msg;
    asprintf(&enc_msg, "NOTICE %s left the chat or crashed", head->userObj->user_name);
    encrypt_msg(enc_msg);
      ...
      FD_SET(0, &readset);
        ...
        if (select_retval > 0) {
                ...
                if(local_user->is_leader == 1) {
                    ...
                    if(used == 1) {
                      ...
                      exit(1);
                        ...
                        broadcast_user_list();
                        int msg_id = ++messageIdCounter;
                      //  char *enc_msg = malloc(strlen(payload)+1);
                      //  strncpy(enc_msg,payload,strlen(payload)+1);
                      //  //encrypt_msg(enc_msg);
                        add_msg_to_queue(payload, undeliveredQueue, msg_id);
            ...
            if(strcmp(msg_type, "list") == 0) {
                ...
                if(local_user->isHoldingElection == 1) {
                    ...
                    while(currMsg != NULL) {
                        ...
                        if (isNotInDelivered) {
                            char *enc_msg = malloc(strlen(currMsg->msg)+1);
                            strncpy(enc_msg,payload,strlen(currMsg->msg)+1);
                            encrypt_msg(enc_msg);
                            add_msg_to_queue(enc_msg, sendQueue, currMsg->msg_id);
                        }
          ...
          if(FD_ISSET(0, &readset)) {
             ...
             char *enc_msg;
             strtok(buffer, "\n");
             asprintf(&enc_msg,"%s%s", getPayload(local_user->user_name, "::"), buffer);
             encrypt_msg(enc_msg);
             asprintf(&payload, "message-request\n%s\n", enc_msg);
             int msg_id = ++messageIdCounter;
             add_msg_to_queue(payload, undeliveredQueue, msg_id);
