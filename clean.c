...
void send_ping_ack(struct sockaddr_in sender_socket) {
...
//thread function for checking client aliveness
...
//thread function for checking server aliveness
...
int main(int argc, char* argv[]) {
    ...
    if(argc != 3 && argc != 4) {
		printf("Usage: ./dchat (NAME USER_PORT) | (NAME COORDINATOR_IP:COORDINATOR_PORT USER_PORT)\n");
		exit(1);
	}
    ...
    local_user->user_socket.sin_port = htons(atoi(argv[argc-1]));
    ...
    ioctl(local_socket_fd, SIOCGIFADDR, &ifr);
    local_user->user_socket.sin_addr = ((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr;
    
    //bind socket to port
    if(bind(local_socket_fd, (struct sockaddr *)&(local_user->user_socket), size_of_socket) == -1) {
		perror("Bind Error");
		exit(1);
	}
        ...
        if (select_retval > 0) {
            ...
            if (FD_ISSET(local_socket_fd, &readset)) {
        		...
        		if(strcmp(msg_type, "ping") == 0) {
        		    handle_ping(msg);
        		    send_ping_ack(sender_socket);
        		}
        		
        		//if ping-ack - update time last alive of coordinator
